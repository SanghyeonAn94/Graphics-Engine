// Core.hpp - 모듈을 지원하지 않는 환경을 위한 헤더 버전
#pragma once
...
namespace Engine {
    ...
    // RenderableObject 객체는 렌더링 관련 함수들을 가져야 함
    template<typename T>
    concept RenderableObject = requires(T obj) {
...
namespace Engine::Utils {
    ...
    ComponentTypeID getComponentTypeID() noexcept {
        ...
        // 타입 해시 코드 사용 (런타임)
        return static_cast<ComponentTypeID>(typeid(T).hash_code());
    }
}
